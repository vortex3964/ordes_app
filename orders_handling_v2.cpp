//orders handling c++ v2--file handling and cost calculation
#include <iostream>
#include <ctime>
#include <chrono>
#include <string>
...
int main() {
	std::cout << "order handling app v2\n";
	std::cout << "=====================\n";
	...
	if (p.is_open()) {
		...
		if (!buffer.empty()) {
			try {
				...
				std::cerr << "Invalid number format for gains.\n";
			}
			catch (const std::out_of_range& e) {
				std::cerr << "Number out of range for gains.\n";
			}
		}
		...
		if (!buffer.empty()) {
			try {
				...
				std::cerr << "Invalid number format for losses.\n";
			}
			catch (const std::out_of_range& e) {
				std::cerr << "Number out of range for losses.\n";
			}
		}
