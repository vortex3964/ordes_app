//orders handling c++ v4--improving live feed and file handling
#include <iostream>
#include <ctime>
#include <chrono>
#include <string>
...
public:
	...
	void refresh() {
		...
		rcv.wait(lock, [] { return ref || stop.load(); });
...
int main() {
	...
	while (run) {
		...
		case 'x':
			...
			stop.store(true);
			cv.notify_all();
			rcv.notify_all();//notify all threads that the proghram is closing
			if(st.joinable()) st.join();
			if (dt.joinable()) dt.join();//wait for all the others threads to finish
			save_profit();
			break;
		case '+':
			add_order(pending);
		...
		case 's':
			...
			break;
		case '$':
			std::cout << "cost:" << losses << "\t" << " gains:" << gains << "\t" << "profit:" << gains - losses << "\n";// money made today
			break;
		case '0'://secret function to add orders fast for testing
			defaultOrder(pending);//adds a default order to pening for testing
			break;
		default:
			std::cout << "\n\033[1m\033[38;2;204;0;51mSorry thats not an option please try another one\033[0m\n";
			break;
		}
		system("pause");
		refresh();
	...
	std::cout << "\n\033[1m\033[38;2;50;205;50mProgram is closing successfully\033[0m\n";
...
void text() {
	std::cout << "\033[38;2;45;131;206m"<<"Order handling app v4\n";
	std::cout << "=====================\n";
	std::cout << "\r\033[38;2;204;0;204mpending:" << p + 1<<"\033[0m";
	std::cout << "\033[38;2;160;160;160m\033[1m \\ \033[0m";
	std::cout << "\033[38;2;102;0;204mready:" << r + 1<< "\033[0m";
	std::cout << "\033[38;2;160;160;160m\033[1m \\ \033[0m";
	std::cout<< "\033[38;2;0;153;76mfinished:" << f + 1 << "\033[0m\n";
	std::cout << "\nplease select one of the following options you would like to do\n";
	std::cout << "----------------------------------------------------------------\n";
	std::cout << "x -> closes the program\n";
	std::cout << "0 -> adds a default pre filled order\n";
...
void save_profit() {
	...
	else std::cout << "\nsomething went wrong in opening bank.txt\n";
	...
	else std::cout << "\nsomething went wrong in closing bank.txt\n";
	p.close();// Close file after reading
}
...
void exportdata(Order* pending, Order* ready, Order* finished) {
	...
	if (import_was_called) writter.open("pending.txt", std::ios::out | std::ios::trunc);
	else writter.open("pending.txt", std::ios::app);
	...
	if (import_was_called) writter.open("ready.txt", std::ios::out | std::ios::trunc);
	else writter.open("ready.txt", std::ios::app);
	...
	if (writter.is_open()) {
		...
		std::cout << "\nsomething went wrong in opening ready.txt\n";
	}
	writter.close();

	if (import_was_called) writter.open("delivered.txt", std::ios::out | std::ios::trunc);
	else writter.open("delivered.txt", std::ios::app);
	...
	if (writter.is_open()) {
		...
		std::cout << "\nsomething went wrong in opening finished.txt\n";
	}
	writter.close();
}
...
void add_order(Order*& order) {
	...
	while (print_menu) {
		menu();
		std::cout << "\nenter your option by picking a number:";
		char choice;
		std::cin >> choice;
		print_menu = false;
		switch (choice) {
